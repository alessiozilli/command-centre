/* ═══════════════════════════════════════════════════════════════
   ...
   v2.4 — 2026-04-28 (bus #259 — volume mute fix; per-item ⏹ stop btn; strip ⏹ Stop btn; Bugs nav tab)
   v2.3 — 2026-04-28 (Plan 7dc6ce70 — config-driven drawer + command strip)
   v2.2.3 — 2026-04-28 (fix: catch-block strip hid toggle; drawer-before-strip order; isConnected guard; task 0d31d2af)
...
(function () {
  ...
  function _setVolume(val) {
    var _sv = parseFloat(val);
    _voiceVolume = isNaN(_sv) ? 0.85 : Math.max(0, Math.min(1, _sv));
    writeLS(KEY_VOICE_VOLUME, _voiceVolume);
    if (_voiceChangeCb) _voiceChangeCb('voice_volume', _voiceVolume);
  }

  function _hideAllStopBtns() {
    document.querySelectorAll('.cc-speak-stop-btn').forEach(function (s) { s.style.display = 'none'; });
  }

  function _showAllStopBtns() {
    document.querySelectorAll('.cc-speak-stop-btn').forEach(function (s) { s.style.display = ''; });
  }
  ...
  function _initSpeakButtons() {
    ...
    function injectBtn(el) {
      ...
      stopB.className = 'cc-speak-stop-btn';
      stopB.textContent = '⏹';
      stopB.title = 'Stop speaking';
  ...
  function _initMobileDrawer() {
    var _exBd = document.getElementById('cc-mobile-backdrop');
    if (_exBd && _exBd.isConnected) return; // already live
  ...
  function _initCommandStrip() {
    ...
    if ('speechSynthesis' in window) {
      ...
      var stopVoiceBtn = el('button', {
        cls: 'cc-strip-btn',
        text: '⏹ Stop',
        attrs: { title: 'Stop current speech' },
        on: { click: function () { window.speechSynthesis.cancel(); _hideAllStopBtns(); } }
      });
      controls.appendChild(stopVoiceBtn);
    ...
    // Move system controls from banner-actions to strip — spare the mobile toggle
    document.querySelectorAll('.cc-banner-actions button:not(#cc-mobile-toggle)').forEach(function (b) { b.style.display = 'none'; });
